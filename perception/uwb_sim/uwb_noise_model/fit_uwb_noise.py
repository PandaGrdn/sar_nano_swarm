#!/usr/bin/env python3
"""fit_uwb_noise.py — fit UWB range/angle noise from ETH-PBL rotation dataset.

Reads ETH-PBL/UWB_DualAntenna_AoA rotation logs (raw .log.gz or preprocessed .h5)
and emits YAML noise parameters for configs/sensors/uwb_pdoa.yaml.

Dataset (default):
  perception/uwb_sim/uwb_noise_model/dataset/Raw Measurement Logs/

Usage:
  python3 perception/uwb_sim/uwb_noise_model/fit_uwb_noise.py
  python3 perception/uwb_sim/uwb_noise_model/fit_uwb_noise.py --write-config

NOTE: STATIC / LOS / tripod-mounted lab data — not flight vibration or NLOS.
  Keep 2x-noise eval discipline until Phase-11 bench data exists.
"""
...
def emit_yaml(
    ...
    lines = [
        ...
        f"  range_sigma_m: {range_fit['range_sigma_m']:.4f}",
        "  range_by_distance_m:",
    ]
    ...
    lines.extend(
        [
            ...
            f"sigma_range_los_m: {range_fit['range_sigma_m']:.4f}",
            f"range_bias_m: {range_fit['range_bias_m']:.4f}",
...
def main():
    ...
    ap.add_argument(
        "--h5",
        default="",
        help="Optional preprocessed HDF5 (processed_data_cache_rotation.h5)",
    )
    ap.add_argument(
        "--antenna-spacing-m",
        type=float,
        default=DEFAULT_ETH_ANTENNA_SPACING_M,
        help="ETH module antenna baseline for PDoA->angle (m)",
    )
    ap.add_argument(
        "--pdoa-sign",
        type=float,
        default=DEFAULT_PDOA_SIGN,
        help="Sign applied to PDoA phase before arcsin (dataset body frame)",
    )
    ap.add_argument(
        "--bin-deg",
        type=float,
        default=5.0,
        help="Angle bin width for off-boresight sigma lookup (deg)",
    )
    ap.add_argument(
        "--output",
        default=os.path.join(_SCRIPT_DIR, "fitted_uwb_noise.yaml"),
        help="Write YAML snippet here",
    )
    ap.add_argument(
        "--write-config",
        action="store_true",
        help="Patch configs/sensors/uwb_pdoa.yaml noise section",
    )
    args = ap.parse_args()

    if args.h5 and os.path.isfile(args.h5):
        print(f"[fit] loading HDF5 {args.h5}")
        true_d, true_a, meas_d, meas_a = _try_load_h5(
            args.h5, args.antenna_spacing_m, args.pdoa_sign
        )
    else:
        ...
        if os.path.isfile(h5_guess):
            ...
            true_d, true_a, meas_d, meas_a = _try_load_h5(
                h5_guess, args.antenna_spacing_m, args.pdoa_sign
            )
        else:
            true_d, true_a, meas_d, meas_a = _load_rotation_logs(
                args.dataset, args.antenna_spacing_m, args.pdoa_sign
            )

    print(f"[fit] total samples: {len(true_d)}")
    range_fit = fit_range(true_d, meas_d)
    angle_fit = fit_angle(true_a, meas_a, bin_deg=args.bin_deg)

    fitted_at = dt.date.today().isoformat()
    yaml_text = emit_yaml(
        range_fit, angle_fit, args.antenna_spacing_m, args.pdoa_sign, fitted_at
    )

    print("\n--- Range fit (demeaned dist_mm residuals) ---")
    print(f"  bias={range_fit['range_bias_m']:.4f} m  sigma={range_fit['range_sigma_m']:.4f} m")
    print("  per distance (m): bias / sigma / n")
    for dist, stats in sorted(range_fit["by_distance_m"].items(), key=lambda x: float(x[0])):
        print(
            f"    {dist:>4}: {stats['bias_m']:+.4f} m / {stats['sigma_m']:.4f} m / n={stats['n']}"
        )

    print("\n--- Angle fit ---")
    pl = angle_fit["piecewise_linear"]
    print(
        f"  sigma(theta) ~ {pl['a_deg']:.2f} deg + {pl['b_deg_per_deg']:.4f} deg/deg * |theta|"
    )
    print(f"  aoa_fov_deg={angle_fit['aoa_fov_deg']:.1f}")
    print(f"  paper FOV sigma={angle_fit['paper_fov_sigma_deg']:.2f} deg")
    print(f"  RMS error (all)={angle_fit['err_rms_deg']:.2f} deg")
    ...
    if args.write_config:
        ...
        _patch_config(cfg_path, range_fit, angle_fit, fitted_at, args.antenna_spacing_m)
        print(f"[fit] patched {cfg_path}")


def _patch_config(cfg_path, range_fit, angle_fit, fitted_at, antenna_spacing_m):
    with open(cfg_path, encoding="utf-8") as f:
        lines = f.readlines()

    pl = angle_fit["piecewise_linear"]
    replacements = {
        "sigma_range_los_m:": f"sigma_range_los_m: {range_fit['range_sigma_m']:.4f}",
    ...
    for line in lines:
        ...
        if stripped.startswith("angle_sigma_slope_deg_per_deg:"):
            ...
            have_slope = True
            continue
        if stripped.startswith("range_bias_m:"):
            out.append(f"range_bias_m: {range_fit['range_bias_m']:.4f}\n")
            have_bias = True
            continue
        if stripped.startswith("noise_calibration:"):
            have_cal = True
        replaced = False
    ...
    if not have_bias:
        idx = next(i for i, l in enumerate(out) if l.strip().startswith("sigma_range_los_m:"))
        out.insert(idx + 1, f"range_bias_m: {range_fit['range_bias_m']:.4f}\n")

    if not have_cal:
        block = [
            f"\n# -- noise calibration (fit_uwb_noise.py {fitted_at}) --\n",
            "noise_calibration:\n",
            f'  source: "ETH-PBL UWB_DualAntenna_AoA (Margiani et al. 2023), fitted {fitted_at}"\n',
            '  conditions: "STATIC / LOS / tripod -- not flight/NLOS"\n',
            f"  dataset_antenna_spacing_m: {antenna_spacing_m:.4f}\n",
        ]
        out.extend(block)
