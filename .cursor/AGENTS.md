...
- `Simulation_Training_Optimization_Roadmap_v3_MOONSHOT.md` (2026-07-27) — **current** roadmap, supersedes v2. Retires the dropped-anchor-puck architecture; UWB becomes an inter-drone range+bearing (PDoA) mesh gauged at a fixed entrance node; adds a Phase 0 MuJoCo install/validation item (perching, Ph 6). `Simulation_Training_Optimization_Roadmap_v2.md` is kept for history — don't treat its UWB/anchor sections as current.
...
- Anchor positions are **state variables, never known constants** — every anchor is instantiated at the dropping drone's *drifted* estimated pose and refined (multi-pose ranging, anchor-to-anchor ranging, loop closures). Any code/eval that treats dropped-anchor positions as ground truth is wrong by design (ground-truth anchors are allowed only as the Phase 2(a) baseline sanity check). **v3 update:** this drone↔anchor architecture is retained only as a frozen, one-time control-arm comparison (v3 roadmap Phase 2 item 1) — it is no longer the primary UWB architecture. The primary mechanism is now an **inter-drone UWB PDoA mesh** (range+bearing between drones, gauged off one fixed entrance/base-station node whose pose *is* a known constant, unlike dropped anchors). No infrastructure is dropped inside the structure anymore.
...
- **Phase 1 M4 (UWB), REDEFINED by v3 — not started.** Split into **M4a** (v2's drone↔anchor analytic range node, kept only as a frozen one-time control-arm run) and **M4b** (v3 primary: inter-drone UWB PDoA — a gz-sim C++ System plugin emitting per-drone-pair range+azimuth+elevation in the observer's body frame, with an AoA field-of-view cone, off-boresight angle-error growth, NLOS/multipath, and a fixed entrance-node gauge). M4b is the next real implementation task — see `.cursor/docs/Phase1_Physical_Fidelity_and_Sensor_Implementation_Plan.md` §5.5.
- Next major work: M4b (inter-drone UWB PDoA plugin), then M5 (Phase-1 exit gate: ±10 cm hold under noise+turbulence), then the non-linear battery model (**never the default linear drain**) and radar degradation model from real bench data. Separately (not blocking this phase): v3 flags installing/validating **MuJoCo** as a Phase 0 checklist item for the later perching gate (Ph 6) — not yet done.
