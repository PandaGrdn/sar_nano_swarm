...
- `Simulation_Training_Optimization_Roadmap_v3_MOONSHOT.md` (2026-07-27, M4 revised 2026-07-30) — **current** roadmap, supersedes v2. Retires dropped anchor pucks; UWB is an inter-drone PDoA mesh gauged at a fixed entrance node; **M4a (drone↔anchor control arm) cancelled**; landed drones serve as static mesh peers if interior references are needed. Adds a Phase 0 MuJoCo install/validation item (perching, Ph 6). `Simulation_Training_Optimization_Roadmap_v2.md` is kept for history — don't treat its UWB/anchor sections as current.
- Technical plan (ethan_technical_plan) — system architecture: 3-tier fleet, anchor-SLAM, auctions, victim detection. ⚠ Its own "Phase 1–5" numbering is a *different* scheme from the roadmap's Phase 0–11. When the user says "Phase N," assume the **roadmap** numbering unless context says otherwise. ⚠ Its anchor-SLAM description also predates v3 — same caveat as above.
- `.cursor/docs/swarm_localization_plan.md` (2026-08-20) — implementation plan for **roadmap Phase 2** (entrance-gauged relative-pose mesh: RIO + UWB range/bearing distributed estimator). **Takes precedence over the roadmap's Phase 2 section where they conflict** (user decision 2026-08-20) — the roadmap section predates the M4 sensor model. Known divergences, both intentional: distributed filter first with the centralized solver as an offline reference (roadmap Ph 2 item 6 said centralized first), and no baro/ToF/flow fusion (roadmap Ph 2 items 1, 8). ⚠ Milestones there are `P2-1…P2-8`; **`M5` in this file and the roadmap is the unrelated Phase-1 hover gate**, not swarm localization.
...
- **v3 UWB architecture (2026-07-30):** no dropped anchor pucks; no M4a drone↔anchor sim. The primary mechanism is an **inter-drone UWB PDoA mesh** (range+bearing between drones, gauged off one fixed entrance/base-station node whose pose *is* a known constant). If fixed interior reference points are needed, **landed drones** act as static mesh peers — same PDoA math as flying neighbors. Legacy v2 anchor-SLAM text in older docs is historical only; do not implement `uwb_range_node` / `uwb_anchors.yaml`.
...
| `perception/` (new subdirs) | — | Radar degradation model, EKF/RIO, place-recognition encoder, vitals pipeline (roadmap Ph 1–5); `flow_sim/` (M3b); `uwb_sim/` (M4) |
...
- **Phase 1 M1 (mass/inertia model): DONE, revised for v3 (2026-07-27).** `configs/airframe/payload.yaml` (base + per-component point masses) + `eval_scripts/apply_payload.py` (parallel-axis mass/CoM/inertia rewrite of the generated SDF's `base_link`, called from `phase0_gate.sh` after radar injection — submodule untouched) + `eval_scripts/thrust_margin_check.py` (T/W gate, floor 1.5 in `configs/airframe/thrust_margin.yaml`, logs to MLflow). **v3 revision:** retired the `anchor_pucks` placeholder entirely (no anchors dropped anymore) and relabeled `uwb_loco_deck` (3.3g, single-antenna, range-only) → `uwb_pdoa_module` (⚠ placeholder 4.0g, dual-antenna, needed for the v3 inter-drone PDoA bearing measurement). Re-verified live: total mass 33.1g → 33.8g, T/W 2.062 → 2.019, still comfortably PASS. Enabling the placeholder radar+GAP9 (no pucks anymore) still drops T/W below the floor — the Brushless-motor question remains real. See `.cursor/docs/Phase1_Physical_Fidelity_and_Sensor_Implementation_Plan.md`.
- **Phase 1 M2 (PID retune, cascade Optuna search): DONE, run live.** `sim_worlds/phase1_pid_tune.sdf` (flat/open world), `configs/airframe/{pid_gains_stock,pid_tune}.yaml`, `eval_scripts/{pid_gains,tune_pid,push_pid_gains,hover_gate}.py`. `tune_pid.py` got a `SIGALRM` watchdog + consecutive-timeout abort partway through (the `cf2` SITL binary crashes/hangs intermittently under long Optuna runs — teardown must kill `gz sim` too, since it holds the UDP ports `cf2` needs). Best Stage-1+2 gains committed to `configs/airframe/pid_gains_loaded.yaml`; `hover_gate.py` (M2 exit gate: RMS horizontal drift + mean |Z error| over a hold window) passes on the loaded-mass model with these gains. **v3 sanity check (2026-07-27):** re-ran `hover_gate.py` only (no re-tune, mass delta was only +0.7g) against the v3-revised mass model — still PASS (mean |Zerr|=1.18cm, RMS horiz drift=8.03cm). Existing gains remain valid.
- **Phase 1 M3 (IR ToF `gpu_lidar` sensor): DONE, gated live, unaffected by v3.** `configs/sensors/tof.yaml`, `eval_scripts/apply_tof_sensor.py` (injects the sensor into `base_link`, called from `phase0_gate.sh` after `apply_payload.py` — kept as a separate script, not folded into `apply_payload.py`), `eval_scripts/tof_gate.py` (rate + hover-plateau altitude-tracking gate). Live result: `/cf_0/tof_down` at ~29.9 Hz (floor 24 Hz), altitude error 0.8–1.8 cm at 0.3/0.6/1.0 m PID-held plateaus. `phase0_gate.sh` also auto-launches `ros_gz_bridge` for this topic when available (see §7 — it's provided by a separately source-built workspace, not the apt package). See `.cursor/docs/Phase1_Physical_Fidelity_and_Sensor_Implementation_Plan.md` §4.1 for live-run gotchas (sdformat's `<lidar><noise><type>` must be a child element, not an attribute like `<air_pressure>` uses; `gz topic -e --json-output` serializes `inf`/`nan` as JSON strings; `gz topic -f --duration` doesn't self-terminate).
- **Phase 1 M3b (PMW3901 optical flow, Flow deck v2 only): DONE, gated live (2026-07-28).** `configs/sensors/optical_flow.yaml` (separate from ToF — PMW3901 ≠ VL53L1x), `perception/flow_sim/flow_node.py` (analytic node + `--selftest`), `eval_scripts/flow_gate.py` (M3b exit gate). `phase0_gate.sh` bridges `/cf_<id>/odom` when `--no-flow` is unset and launches the flow node with `python3 -u`. Live gate (`flow_gate.py` on `phase1_pid_tune --no-radar --headless`): `/cf_0/flow` @ ~100 Hz; hover valid fraction 100%, hover RMS ~0.31 m/s (noise floor at `flow_std_px=2.0` driver units); forward RMSE vs `debug_truth` ~0.32 m/s, vx bias ~−0.02 m/s; smooth-patch dropout valid fraction ~8%. **Fidelity caveat:** surface texture map is hand-authored (Tier B), not measured from real floors. **`/cf_<id>/flow/debug_truth` is a sim oracle — Tier A: the estimator must never subscribe.** Node exits 2 if `deck != flow_v2` in `tof.yaml`. See `.cursor/docs/M3b_Optical_Flow_Implementation_Plan.md`.
- **Phase 1 M4 (UWB inter-drone PDoA mesh) — DONE, gated live (2026-07-30).** **M4a cancelled** (no pucks). **Authority:** `.cursor/docs/M4_UWB_Relative_Positioning_Implementation_Plan.md` (supersedes parent §5 C++ plugin sketch). **Implementation reversal (§1.1):** Python analytic node (`perception/uwb_sim/{uwb_model,uwb_edges,uwb_node}.py`), not a gz-sim C++ plugin. **Airtime model (§1.3):** one swarm-wide node, round-robin scheduler, `max_exchanges_per_s` + `max_neighbors_per_drone`. Config: `configs/sensors/uwb_pdoa.yaml`. Offline: `uwb_model.py --selftest` (17/17 PASS). `phase0_gate.sh`: `-n N`, `--spacing`, `--no-uwb`, `--uwb-config`; kills stale `uwb_node` on start. Topics: `/cf_<id>/uwb/edges`, `/uwb/edges_all`, `/uwb/peer_<id>/edges` (static peers — ROS rejects numeric topic tokens), `/uwb/edges_truth` (sim oracle — Tier A). Gate: `eval_scripts/uwb_gate.py` **PASS 12/12** on `phase1_pid_tune -n 2 --spacing 2.0 --no-radar --headless --no-flow`: range err mean 0.08 m, bidir range identity 100%, bearing-valid A 100%, az RMS 8.5°, rel-pos err mean 0.38 m, AoA-cone degrade + entrance node seen. NLOS live not gated (offline selftest checks 8–10). M4-4 (landed peers + mesh LOS) not run — config-only, low priority.
- Next major work: **M5** (Phase-1 exit gate: ±10 cm hold under noise+turbulence), **M2b** disturbance flags, then non-linear battery model (**never the default linear drain**) and radar degradation model from real bench data.
...
- **Dev machine's shell is PowerShell, not bash** — when running `wsl -e bash -lc "..."` from the host, double-quoting the inner bash script lets PowerShell interpolate `$?`/`$var` *before* wsl ever sees them (e.g. `echo EXIT=$?` silently becomes `echo EXIT=True`/`False`, PowerShell's own last-command-success flag, not bash's exit code). Always single-quote the outer `bash -lc '...'` argument when the inner script uses `$`-anything.
- **Python's stdout is fully buffered (not line-buffered) when redirected to a file/pipe** — a script killed by `timeout`/`SIGTERM` before it exits normally can lose 100% of its buffered output even though it ran fine and would have passed. Use `python3 -u` (or `PYTHONUNBUFFERED=1`) whenever redirecting a gate script's output to a log file, especially under a `timeout` wrapper.
- **Optical-flow node ToF staleness:** compare receive wall time to `tof_stale_s`, not `get_clock().now()` vs the LaserScan header stamp — sim time and ROS wall time diverge and every height sample looks stale (flow stays invalid forever).
...
- **UWB gate: kill stale `uwb_node` / `gz sim` before runs** — duplicate `uwb_node` processes inflate edge-rate counts; `phase0_gate.sh` now pkill's them on start. **`uwb_gate.py` must be the first cflib connection** on a fresh sim (CRTP v7 one-shot arming). Static peer topics use `/uwb/peer_<id>/edges` (not `/uwb/1000/edges` — invalid ROS 2 topic name).
...
- **Coplanar mesh geometry in evals** — corridor chains with poor vertical DOP are the degenerate case to test honestly; don't use idealized wall/ceiling anchor placements that flatter results. (Dropped puck evals are retired with M4a.)
