...
- `configs/sensors/{tof,uwb,uwb_anchors}.yaml`  (thermal deferred, §4.2)
...
- **ROS↔gz bridging** — the ToF `gpu_lidar` topic must be bridged to ROS 2; verify the existing bridge config and extend it (don't assume it auto-appears).
