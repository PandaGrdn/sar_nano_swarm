...
2. **Comms is not the moonshot.** Deleting anchor pucks deletes the *localization* backbone, not the RF physics. Comms in v3 = landed drones as relays + rendezvous/data-mule for bulk data. Class-A alert latency from deep inside will be worse than the v2 backbone design; this is an accepted, stated trade.
3. **No dropped infrastructure.** Anchor pucks and the v2 drone↔anchor control arm (**M4a**) are **cancelled** — not implemented in sim or mission software. Fixed reference points inside the structure, if needed, are **landed drones** acting as mesh peers (same UWB PDoA math as any other drone). Publishability rests on the inter-drone mesh + loop-closure results, not a frozen puck baseline.
...
| **Downward optical flow (Flow deck)** | Use the **same Flow deck v2** for horizontal velocity aiding local odometry, alongside down-ToF for altitude | Phase 2 (EKF fusion) | Real deck = VL53L1x (ToF) **+ PMW3901** (optical flow) on one board. **Phase 1 M3b sim is built and gated** (`flow_gate.py`); Phase 2 consumes `/cf_<id>/flow` in the local EKF. |

**M4 / UWB architecture (2026-07-30 — aligned with `Phase1_Physical_Fidelity_and_Sensor_Implementation_Plan.md` §5):**

| Decision | Choice | Notes |
|---|---|---|
| **M4a (pucks / drone↔anchor node)** | **Cancelled** | No `uwb_range_node`, no `uwb_anchors.yaml`, no dropped pucks |
| **Fixed interior references** | **Landed drones** | A drone lands/holds and serves as a static mesh peer — not a separate puck payload |
| **AoA cone (decision 8)** | **180° default** | Practical maximum while maintaining usable bearing accuracy; front vs. back is the limiting ambiguity |
| **360° azimuth (side note)** | Yaw twitch | Small deliberate yaw motion + before/after bearing comparison disambiguates forward vs. rear half-plane — future estimator/motion primitive, not M4 scope |
| **Implementation** | **Python analytic node** (`perception/uwb_sim/`) | **DONE, gated (2026-07-30)** — `uwb_gate.py` PASS 12/12; not a C++ gz-sim plugin; see `M4_UWB_Relative_Positioning_Implementation_Plan.md` |
| **Swarm measurement reinforcement (future)** | Range + bearing cross-checks | Inter-drone relative range/bearing can reinforce or correct local odometry drift (e.g. IMU/flow drift corrected by consistent mesh edges) — Phase 2+ fusion work |
...
**Flow deck dual role (meeting 2026-07-28; M3b gated 2026-07-28):** when `configs/sensors/tof.yaml` selects `deck: flow_v2` (current default), the airframe carries both sensors in hardware. Phase 1 M3 sim injects **VL53L1x down-ranging** (M3a) and **PMW3901 optical flow** (M3b) — see `Phase1_Physical_Fidelity_and_Sensor_Implementation_Plan.md`. Phase 2 fuses the flow stream into the local EKF between radar updates and when the UWB mesh is sparse.
...
2. ~~**Inter-drone UWB PDoA measurement plugin**~~ — **DONE (2026-07-30).** Built as Python analytic node `perception/uwb_sim/uwb_node.py` (M4 plan §1.1 reversal — not a C++ plugin). Gate: two SITL drones exchanging range+bearing on ROS topics with range-only AoA-cone fallback — `uwb_gate.py` PASS 12/12.
...
**Gate to exit Phase 0:** v2 gate (SITL drone + radar point cloud in SubT world) **plus** two SITL drones exchanging simulated UWB range+bearing — **UWB half DONE** (`uwb_gate.py` on `phase1_pid_tune -n 2`). SubT + radar end-to-end still to verify.
...
2. **Load the validated UWB model into the Phase 0 node** (`perception/uwb_sim/`), replacing placeholders; apply the radar degradation model on top of `radarays_gz2` output from Phase 2 onward.
...
8. **Path trace / breadcrumb log (new, meeting 2026-07-28).** Each drone logs at every control timestep: `(t, v, Δψ)` — speed scalar (or body-frame velocity), heading change since last step, and timestamp — appended to a ring buffer on-board and mirrored to the squad graph as Class-B bulk data. Integrating `(v, Δψ)` reconstructs the flown path in the drone's local frame; fusing with the mesh estimate places it globally. **Purpose:** when a victim is found, the drone (or a relay) can command **return along the recorded trace** even if global coords are stale — dead-reckoning RTH is the fallback when mesh quality degrades. Implement in `coordination/` as a lightweight logger; gate: integrated breadcrumb path vs ground-truth trajectory RMSE < 2× the front-end odometry error over a 30 s sortie. **Optical flow (M3b done):** Phase 2 fuses the simulated PMW3901 stream (`/cf_<id>/flow`, gated by `flow_gate.py`) into the local EKF for short-horizon velocity integration — breadcrumbs and local hold.

0. **AoA-cone coverage handling (decision 8).** The UWB AoA cone defaults to **180°** — the practical maximum while maintaining usable bearing accuracy, because dual-antenna PDoA on a nano airframe cannot resolve **front vs. back** without extra motion. Out-of-cone neighbors (beyond 180°) contribute range-only edges. Per the graph-rigidity literature (range+angle edges make a subgraph rigid; range-only edges leave rotational freedom), the estimator and behaviors must together ensure enough angle edges exist: model antenna mounting orientation explicitly, evaluate whether corridor chains need alternating mounts or periodic yaw-glances to close mutual-bearing pairs, and have the graph track its own rigidity (flag floppy subgraphs) rather than silently absorbing them.

   > **Side note — 360° azimuth:** full **360° bearing** is possible via a **slight yaw twitch** — deliberate small yaw motion and comparison of bearing readings before/after disambiguates forward vs. rear half-plane. Future estimator/motion primitive; not Phase 1 M4 scope.

   > **Future work — swarm measurement reinforcement:** inter-drone **relative range and bearing** can **reinforce or correct local positioning drift** — if one drone's IMU/flow odometry drifts, consistent mesh edges to neighbors provide cross-checks that tighten the local state. Implement as optional fusion factors in Phase 2+.

1. ~~**Baseline condition (retained control arm):** v2's anchor experiments~~ — **CANCELLED (2026-07-30).** M4a puck/anchor sim is not being built. W1 results stand on the inter-drone mesh alone; v2 puck numbers may be cited from literature/historical runs only, not reproduced in-repo.
...
1. **Remove:** puck payload handling and drop-mechanism behaviors. (Payload margin freed — note it, it partially relieves the 2–4 min sortie floor.) **Landed drones** already serve as static mesh/relay nodes — no separate puck hardware.
...
3. **Deliberate-expenditure accounting:** drone-minutes spent as static mesh nodes are now a budgeted mission cost; report victims-found and area-covered *per drone-minute*, so the moonshot architecture can be compared honestly against historical v2 puck-architecture literature if needed (not reproduced in-repo).
...
- **Total: ~6–8 months** before real-hardware field testing is responsible (vs ~4–6 for v2). The extra 2 months buy three paper-shaped results and the moat; Track H remains the hedge if W2 stalls.
