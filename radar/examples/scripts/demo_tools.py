...
def draw_fov(bins):
    ...
    if num_sections < 16:
        color_map = [matplotlib.colormaps['inferno'].resampled(num_sections)(i) for i in range(num_sections)]
        degree_fontsize, section_fontsize = 11, 11
    else:
        plasma_colors = matplotlib.colormaps['plasma'].resampled(num_sections // 2)
        magma_colors = matplotlib.colormaps['inferno'].resampled(num_sections // 2)
        color_map = [
            color
