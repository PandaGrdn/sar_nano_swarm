...
def get_pointcloud(index, seq_dir, params):
  ...
  if params['sensor_type'] == 'lidar':
    filename = os.path.join(seq_dir, 'lidar', 'pointclouds', f'lidar_pointcloud_{index}.bin')
  else:
    filename = os.path.join(seq_dir, 'single_chip', 'pointclouds', 'data', f'radar_pointcloud_{index}.bin')
...
def read_phase_freq_calib(calib_filename):
  ...
  cal = data['antennaCalib']

  num_tx = cal['numTx']
  num_rx = cal['numRx']
  frequency_slope = cal['frequencySlope']
  sampling_rate = cal['samplingRate']

  freq_cal_mat_arr = np.array(cal['frequencyCalibrationMatrix']).astype('float')
  freq_cal_mat = freq_cal_mat_arr.reshape(num_tx, num_rx)

  phase_cal_mat_arr = np.array(cal['phaseCalibrationMatrix']).astype('float')
  phase_cal_mat_arr = phase_cal_mat_arr[0::2] + 1j * phase_cal_mat_arr[1::2]
  phase_cal_mat = phase_cal_mat_arr.reshape(num_tx, num_rx)
