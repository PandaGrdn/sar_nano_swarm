...
from filtering import clustering, log_likelihood_distance, estimate_velocity

# to run:
# python3 examples/python/plot_clusters_animation.py \
#   --seq /Users/monika/Downloads/radar/data/coloradar/kitti/2_23_2021_edgar_classroom_run4 \
#   --calib /Users/monika/Downloads/radar/data/coloradar/calib \
#   --threshold 0.15 \
#   --min_range 0 \
#   --plot_heatmap true \
#   --single_chip false
...
# --- clustering/tracking state (module-level globals) ---
radar_num_threshold = 7
match_dist_threshold = 1.0
cluster_cov = np.diag([0.5, 0.05, 0.05])
track_id_counter = itertools.count()
prev_cluster_centers = []
prev_cluster_ids = []
prev_radar_timestamp = None
dynamic_cluster_artists = []  # centroid scatter + velocity arrows, cleared/redrawn each radar frame
...
def animate_plot(i):
  ...
  global positions
  global ax
  global prev_cluster_centers, prev_cluster_ids, prev_radar_timestamp, dynamic_cluster_artists
  ...
  is_radar_frame = (plot_data[i][3] == 'radar')
  ...
  mask = filter_radar_by_lidar(radar_pc[:, :3], lidar_pc[:, :3], max_dist=0.5)
  radar_pc_filtered = radar_pc
  ...
  plots['radar']._offsets3d = (radar_pc_filtered[:,0],
                            radar_pc_filtered[:,1],
                            radar_pc_filtered[:,2])

  # --- clustering + tracking + velocity, only on radar frames ---
  if is_radar_frame and radar_pc_filtered.shape[0] > 0:
    current_timestamp = plot_data[i][0]

    labels = np.array(clustering(radar_pc_filtered))
    cluster_dict = defaultdict(list)
    for idx in range(len(labels)):
      cluster_dict[int(labels[idx])].append(radar_pc_filtered[idx])

    centers, sizes = [], []
    for label, pts in cluster_dict.items():
      if label == -1 or len(pts) < radar_num_threshold:
        continue
      pts_arr = np.array(pts)
      centers.append(pts_arr[:, :3].mean(axis=0))
      sizes.append(len(pts))

    ids = [None] * len(centers)
    ...
    for ci, center in enumerate(centers):
      ...
      for j, prev_center in enumerate(prev_cluster_centers):
        if j in used_prev:
          continue
        d = log_likelihood_distance(prev_center, center, covariance=cluster_cov)
        if best_dist is None or d < best_dist:
          best_dist, best_j = d, j
      if best_j is not None and best_dist < match_dist_threshold:
        ids[ci] = prev_cluster_ids[best_j]
        used_prev.add(best_j)
        if prev_radar_timestamp is not None:
          vel_vec, speed = estimate_velocity(prev_cluster_centers[best_j], center,
                                              prev_radar_timestamp, current_timestamp)
          velocities[ci] = (vel_vec, speed)
      else:
        ids[ci] = next(track_id_counter)

    # clear last frame's cluster markers/arrows
    for artist in dynamic_cluster_artists:
      artist.remove()
    dynamic_cluster_artists = []

    if centers:
      centers_arr = np.array(centers)
      colors = [cmap(tid % 20) for tid in ids]
      centroid_scatter = ax.scatter(centers_arr[:, 0], centers_arr[:, 1], centers_arr[:, 2],
                 c=colors, s=200, marker='*', edgecolors='black', linewidths=1.2, zorder=10)
      dynamic_cluster_artists.append(centroid_scatter)

      for center, vel in zip(centers, velocities):
        if vel is not None:
          vel_vec, speed = vel
          q = ax.quiver(center[0], center[1], center[2],
                        vel_vec[0], vel_vec[1], vel_vec[2],
                        length=0.5, normalize=True, color='lime', zorder=11)
          dynamic_cluster_artists.append(q)

    ax.set_title(f"{radar_label} | {len(centers)} clusters | t={current_timestamp:.3f}s")

    prev_cluster_centers, prev_cluster_ids, prev_radar_timestamp = centers, ids, current_timestamp
